...
export async function activate(context: vscode.ExtensionContext) {
  ...
  // Initialize clients
  const apiUrl = vscode.workspace.getConfiguration('tivra').get<string>('apiUrl') || 'http://localhost:3001';
...
async function generateFixWithClaude(analysis: ServiceAnalysis | null) {
  ...
  // Send to Claude Code
  const serviceName = analysis.service.name;
  await claudeIntegration.sendToClaudeCode(context, serviceName);

  // Show helpful message
  vscode.window.showInformationMessage(
    `🤖 AWS Runtime Context for ${serviceName} sent to Claude Code!\n\nThe context includes:\n- ${analysis.logs?.totalErrors || 0} CloudWatch errors\n- ${analysis.metrics?.errorRate?.toFixed(1) || 0}% error rate\n- Stack traces and logs\n- AI-generated fix suggestions\n\nClaude Code now has full runtime context + codebase access to generate a fix.`
  );
}
